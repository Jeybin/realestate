...
$today = date("Y-m-d");
$favourite = $functions->getFavouritesByUserIdAndType($loginid,$logintype);
if($favourite){
foreach($favourite as $favproperties){
	$fav_prop_id = $favproperties['propertyid'];
$allproperties = $functions->getAllPropertiesById($fav_prop_id);
?>
...
<?php } } } else { ?>
	<h4 class='text-center' style="opacity:0.5;color:#2d2d2d">No Favourites Added</h4>
<?php } ?>
